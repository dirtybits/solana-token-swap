...
pub struct MakeOffer<'info> {
    ...
    // Work with either the classic token program or the newer token extensions program
    pub token_program: Interface<'info, TokenInterface>,

    // Used to create accounts
    pub system_program: Program<'info, System>,

    #[account(mut)] // mutable because signer will pay for the transaction
    ...
    #[account(
        init, // Throws error if maker account already exists
        payer = maker,
        space = Offer::DISCRIMINATOR.len() + Offer::INIT_SPACE, // space needed for the offer account
    ...
    #[account(
        ...
        payer = maker,
        associated_token::mint = token_mint_a,
        associated_token::authority = offer, // offer data account owns this vault
        associated_token::token_program = token_program
    )]
...
pub fn make_offer(
    context: Context<MakeOffer>, // gives us details of the instruction and accounts
    id: u64,
    token_a_offered_amount: u64,
    token_b_wanted_amount: u64, // provided by incoming instruction
) -> Result<()> {
    // Validate amounts
